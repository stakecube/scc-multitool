...
case $maintstart in
	...
	96)	echo -e "${YELLOW}Beginning collect single node debug log tool${NC}"
		...
		rm ~/${debugzipfilename}
		7za a -t7z -spf -mx=9 -md=64m -mfb=273 -mmt=on -ms=on -- ~/${debugzipfilename} /home/$alias/.scc/debug.log 
    ...
    if [[ $foundone -eq 1 && -n "$debugfilelist" ]]; then
        ...
        7za a -t7z -spf -mx=9 -md=64m -mfb=273 -mmt=on -ms=on -- ~/"$debugzipfilename" $debugfilelist
